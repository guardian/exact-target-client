...
class ListSubscriberMessageEncoder extends MessageEncoder[SubscriptionRequest, Seq[SubscriberResult]] {
      ...
      <soap:Header>
        ...
        <wsse:Security soap:mustUnderstand="1">
          <wsse:UsernameToken xmlns:wsu="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">
            <wsse:Username>{request.accountDetails.username}</wsse:Username>
            <wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText">{request.accountDetails.password}</wsse:Password>
          </wsse:UsernameToken>
        </wsse:Security>
      </soap:Header>
      <soap:Body>
  ...
  def decodeResponse(response: NodeSeq) = {
    (response \\ "CreateResponse" \\ "Results") map { subscriberNode =>
